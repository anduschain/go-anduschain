...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config) (*state.StateDB, common.Hash, error) {
	...
	// Prepare the EVM.
	evm := vm.NewEVM(vm.BlockContext{BlockNumber: big.NewInt(1)}, txContext, nil, params.TestChainConfig, vm.Config{})

	// Execute the message.
	snapshot := statedb.Snapshot()
	gaspool := new(core.GasPool)
	gaspool.AddGas(block.GasLimit())
	if _, err := core.ApplyMessage(evm, msg, gaspool, big.NewInt(0)); err != nil {
...
func (tx *stTransaction) toMessage(ps stPostState, baseFee *big.Int) (core.Message, error) {
	...
	msg := types.NewMessage(from, to, tx.Nonce, value, gasLimit, gasPrice, data, false)
	return msg, nil
}
