...
import (
	...
	l "github.com/anduschain/go-anduschain/les"
...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, deb.NewFaker(otprn), db, testChainLen,
	...
	bc, _ = core.NewBlockChain(db, nil, gspec.Config, deb.NewFaker(otprn), vm.Config{})
...
func makeTries() (chtTrie *trie.Trie, bloomTrie *trie.Trie, chtKeys, bloomKeys [][]byte) {
	chtTrie, _ = trie.New(common.Hash{}, trie.NewDatabase(ethdb.NewMemDatabase()))
	bloomTrie, _ = trie.New(common.Hash{}, trie.NewDatabase(ethdb.NewMemDatabase()))
