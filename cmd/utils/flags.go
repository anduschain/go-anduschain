...
import (
	...
	"github.com/anduschain/go-anduschain/metrics/influxdb"
...
var (
	...
	ZktrieEnableFlag = cli.BoolFlag{
		Name:  "zktrie",
		Usage: "StateDB use ZkTrie",
	}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if cfg.SyncMode == downloader.LightSync {
		err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			backend, err := les.New(ctx, stack, cfg)
			return backend, err
		})
	} else {
		err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			fullNode, err := eth.New(ctx, stack, cfg)
			if fullNode != nil && cfg.LightServ > 0 {
				ls, _ := les.NewLesServer(fullNode, cfg)
				fullNode.AddLesServer(ls)
			}
			stack.SetBackend(fullNode.EthApiBackend())
			return fullNode, err
		})
	}
	if err != nil {
