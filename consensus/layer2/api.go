...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.layer2.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) Proposals() map[common.Address]bool {
	api.layer2.lock.RLock()
	defer api.layer2.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.layer2.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.layer2.lock.Lock()
	defer api.layer2.lock.Unlock()

	api.layer2.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.layer2.proposals, address)
}
...
func (api *API) Status() (*status, error) {
	...
	for n := start; n < end; n++ {
		...
		sealer, err := api.layer2.Author(h)
		if err != nil {
			return nil, err
		}
		signStatus[sealer]++
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.layer2.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.layer2.Author(block.Header())
	}
