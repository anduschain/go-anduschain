...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = memorydb.NewMemDatabase()
		lightdb = memorydb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, deb.NewFullFaker(otprn), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, deb.NewFaker(otprn), fulldb, 4, testChainGen)
