...
import (
	...
	"github.com/anduschain/go-anduschain/rollup/sync_service"
)
...
func (c *L1Client) getLatestFinalizedBlockNumber(ctx context.Context) (uint64, error) {
	// CSW - Anduschain use only LastestBlockNumber
	//header, err := c.client.HeaderByNumber(ctx, big.NewInt(int64(rpc.FinalizedBlockNumber)))
	header, err := c.client.HeaderByNumber(ctx, big.NewInt(int64(rpc.LatestBlockNumber)))
	if err != nil {
		return 0, err
	}
