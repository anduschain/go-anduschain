...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	conBc, _ := NewBlockChain(conDb, nil, &conConf, deb.NewFaker(otprn), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		blocks, _ = GenerateChain(&proConf, conBc.CurrentBlock(), deb.NewFaker(otprn), db, 1, func(i int, gen *BlockGen) {})
		if _, err := conBc.InsertChain(blocks); err == nil {
			//t.Fatalf("contra-fork chain accepted pro-fork block: %v", blocks[0])
		}
		// Create a proper no-fork block for the contra-forker
		blocks, _ = GenerateChain(&conConf, conBc.CurrentBlock(), deb.NewFaker(otprn), db, 1, func(i int, gen *BlockGen) {})
		if _, err := conBc.InsertChain(blocks); err != nil {
		...
		blocks, _ = GenerateChain(&conConf, proBc.CurrentBlock(), deb.NewFaker(otprn), db, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err == nil {
			//t.Fatalf("pro-fork chain accepted contra-fork block: %v", blocks[0])
		}
		// Create a proper pro-fork block for the pro-forker
		blocks, _ = GenerateChain(&proConf, proBc.CurrentBlock(), deb.NewFaker(otprn), db, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err != nil {
