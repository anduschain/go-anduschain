...
// test all variations.
func makeSMTProvers(mt *ZkTrie) []func(key []byte) *memorydb.MemDatabase {
	var provers []func(key []byte) *memorydb.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *memorydb.MemDatabase {
...
func TestSMTOneElementProof(t *testing.T) {
	...
	for i, prover := range makeSMTProvers(tr) {
		...
		val, _, err := VerifyProof(common.BytesToHash(mt.Root().Bytes()), keyBytes, proof)
		if err != nil {
...
func TestSMTProof(t *testing.T) {
	...
	for i, prover := range makeSMTProvers(mt) {
		for _, kv := range vals {
			...
			val, _, err := VerifyProof(common.BytesToHash(root.Bytes()), kv.k, proof)
			if err != nil {
...
func TestSMTBadProof(t *testing.T) {
	...
	for i, prover := range makeSMTProvers(mt) {
		for _, kv := range vals {
			...
			if _, _, err := VerifyProof(common.BytesToHash(root.Bytes()), kv.k, proof); err == nil {
