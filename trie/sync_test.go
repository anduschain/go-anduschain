...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(memorydb.NewMemDatabase())
	dbB := NewDatabase(memorydb.NewMemDatabase())
	emptyA, _ := New(common.Hash{}, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), memorydb.NewMemDatabase(), nil).Missing(1); len(req) != 0 {
