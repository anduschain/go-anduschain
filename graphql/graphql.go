...
func (b *Block) VoterHash(ctx context.Context) (common.Hash, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return common.Hash{}, err
	}
	return header.VoteHash, nil
}

func (b *Block) VoterCount(ctx context.Context) (*int32, error) {
	block, err := b.resolve(ctx)
	if err != nil || block == nil {
		return nil, err
	}
	count := int32(len(block.Voters()))
	return &count, err
}

func (b *Block) Voters(ctx context.Context) (*[]common.Address, error) {
	block, err := b.resolve(ctx)
	if err != nil || block == nil {
		return nil, err
	}
	ret := make([]common.Address, 0, len(block.Voters()))
	for _, voter := range block.Voters() {
		ret = append(ret, voter.Voter)
	}
	return &ret, nil
}
